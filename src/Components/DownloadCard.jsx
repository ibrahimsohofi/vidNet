...
function DownloadCard({ videoInfo, videoOpt, audioOpt, inputValue }) {
  ...
  return (
    <section className="bg-white flex justify-between m-3 relative p-3 border w-[98%] rounded-lg">
      ...
      <article className=" w-[59.6%]  h-7/12 border-gray-400 border p-2 rounded-lg  ">
        ...
        <div className="flex w-full  ">
          <div className=" items-center flex w-6/12 bg-slate-600 rounded-t-lg ">
            <button
              onClick={() => {
                handleShowOption("video");
              }}
              className={`${
                activeOption === "video" ? "activeOpt" : "notActive "
              } ${
                activeOption === "cover" ? "rounded-br-none" : "rounded-br-lg"
              }  rounded-tl-lg isOptActive `}
            >
              Video
            </button>
            <button
              ...
              className={`${
                activeOption === "audio" ? "activeOpt" : "notActive"
              } ${
                activeOption === "cover"
                  ? "rounded-bl-none rounded-br-lg border-l-slate-600 "
                  : "rounded-bl-lg"
              } isOptActive`}
            >
              Audio
            ...
            <button
              ...
              className={`${
                activeOption === "cover"
                  ? "activeOpt"
                  : "notActive border-r-2 border-r-slate-600"
              } ${
                activeOption === "audio"
                  ? "rounded-bl-lg border-l-0"
                  : "rounded-bl-0"
              } isOptActive rounded-tr-lg`}
            >
              Thumbnail
            </button>
          </div>
        ...
        <div className="w-full videoOption  border-x-slate-600 border-b-slate-600 border-b-2 rounded-tr-lg rounded-b-lg border-x-2">
          {activeOption === "video" && (
            <HandleVideoInfo
              videoOpt={videoOpt}
              videoInfo={videoInfo}
          ...
          {activeOption === "cover" && (
            <HandleCoverInfo videoInfo={videoInfo} />
          )}
        </div>
      </article>
    </section>
